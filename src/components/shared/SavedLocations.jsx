    ...
    <>
      {places.length > 0 && (
        <div className="flex items-center justify-center h-full overflow-hidden ">
          <Slider {...sliderSettings} className="w-full md:w-5/12">
            {places.map((place) => (
              <div key={place._id} className="px-4 mt-10">
                <div
                  className="bg-gray-100 p-4 rounded-md shadow-md relative h-14 w-full flex items-center justify-center cursor-pointer"
                  onClick={() => handleLocationClick(place)} 
                >
                  <h3 className="text-sm font-semibold mb-0 truncate">
                    {place.title}
                  </h3>
                  <button
                    onClick={() => handleDelete(place._id)}
                    className="absolute top-2 right-2 w-6 h-6 opacity-0 hover:opacity-100 transition duration-300 flex items-center justify-center"
                  >
                    <IoMdClose />
                  </button>
                </div>
